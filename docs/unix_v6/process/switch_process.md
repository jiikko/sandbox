...
## もっと雑にこういうことやってる
* sleep()を実行するとプロセスの切り替えが起きている
  * 自身を休眠状態にしてプロセスを切り替える
    * 次の実行するプロセスは優先度と実行可能状態の状態のプロセスから選ぶ
    * プロセスを切り替える時は、コンテキストスイッチ(レジスタとメモリ領域の引っ越し)が起きている
...
* waitシステムコールには、子プロセスの処理完了を待つために実行プロセスの中断すること以外にも、終了した子プロセスの後始末を行うという役割がある
  * 子プロセスがexitするとゾンビ状態になって親がwaitするまでproc[]に存在し続ける. waitするとproc[]から消える
```shell
...
* プロセスの状態を休眠状態から実行可能状態へと変更するwakeup()を実行する
  * wakeup()は、sleep()と同様に変数などのアドレスを引数として受け、proc.p_wchanを順に調べ、引数と一致するプロセスを実行可能状態にする
  * sleep()とwakeup()を使って、複数プロセス間で資源を持ち合う処理を行なっている
* wakeup()で休眠状態プロセスを起こした(実行可能状態にした)後にswtch()でプロセスを切り替える
...
sleep(chan, pri) { // waiting_chanenel: 持っている資源アドレス, pri: 実行優先度
  ...
  if(pri >= 0) {
    if(issig()) { goto psig; } // シグナルを受けているならgoto psig
    spl6();             // 下記3項目が書き換えられない(割り込みされない)ように6に引き上げる
    rp->p_wchan = chan; // 割り込み処理中に実行されるwakeup()から書き換えられる可能性がある
    rp->p_stat = SWAIT; // 同じ休眠状態だけどシグナル処理を行えるSWAIT
    rp->p_pri = pri;
    spl0();
    if(runin != 0) { // スワップアウト対象がいないことを示すruninフラグ. いるなら真
      runin = 0;
      wakeup(&runin);
    }
    swtch();
...
## savu(), retu(), aretu()
* user\[\]とレジスタへの退避と復元を行なっている処理
* スタックのスタックがでてきたりアセンブラがよくわからないので略

## setpri()
* setpri() はプロセスの優先度を更新する
* 実行タイミングは、クロック割り込みやシグナル処理
  * 定期的に実行優先度が更新されている
* proc.p_priは低いほうが優先度が高い
  * 最高値は127
* proc.p_cpuがそのプロセスが使用したCPUの累積時間で、そのプロセスの実行中にクロック割り込みがかかるとインクリメントされる
  * ただし、スワップインするときに0にリセットされる
* `CPUの使用累積時間が長くなると実行優先度が下がる`
* proc.p_niceはniceシステムコールによって、ユーザから設定される正の値
  * 実行優先度を下げるための補正値
* ただし、スーパーユーザは負の値を設定できる

```c
setpri(up) { // 実行優先度を更新するproc[]エントリ
  register *pp, p;
  pp = up;
  p = (pp->p_cpu & 0377) / 16;
  p =+ PUSER + pp->p_nice;
  ...
  if(p > curpri) { // current priority. 不等号は逆. 次のバージョンで修正されているとのこと
    runrun++;      // runrun は実行プロセスよりも実行優先度の高いプロセスが存在することを表すフラグ
 }
  pp->p_pri = p;
}
```
...
## wakeup()
* wakeup()はproc[]を順番に調べて、引数で指定された資源を持っている休眠状態のプロセスを起こす関数
...
* proc[]を先頭から順にたどり、引数で洗えられた値と同じproc.p_wchanを持つプロセスに対して、setrun()を実行して実行可能状態にする

## setrun()
引数で与えられたプロセスを実行可能状態(SRUN)にする

```c
setrun(p) { // p: 実行可能状態にするproc[]エントリ
  register struct proc *rp;
  rp = p;
  rp->p_wchan = 0; // スリープしている理由がセットされており、これから実行可能状態になるので理由をリセットする
  rp->p_stat = SRUN;
  if(rp->p_pri < curpri) { runrun++; }
  // runoutフラグはスワップインするプロセスがないことを示す
  // runoutフラグが立っていて、かつ、起こされたプロセスがスワップアウトされた状態ならばスワップインできるプロセスが現れたかもしれないので
  // runoutフラグをクリアし、スケジューラを起こす(謎)
  if(runout != 0 && (rp_>p_flag & SLOAD) == 0) {
    runout = 0;
    wakeup(&runout);
  }
}
